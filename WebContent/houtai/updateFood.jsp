...
$(document).ready(function(){
	...
	/* alert("${food.fTypeId }");
	alert("${food.fZhuangTai }");
	alert("${food.fCompanyId }");
	 */
	$("#fTypeId").val("${food.fTypeId }");
	$("#fZhuangTai").val("${food.fZhuangTai }");
	$("#fCompanyId").val("${food.fCompanyId }");
	$("#fZhunRu").val("${food.fZhunRu }");
...
<div>
   <!-- 第一层，添加公司信息的标题 -->
   <div style="margin-left: 45%;font-size: 4;font-weight: bold;">修改${food.fId }号食品的基本信息</div>
   <div style="margin-left: 10%;margin-top: 20px;">
     <form action="updateFood.do" method="post" enctype="multipart/form-data" onsubmit="return jiaoyan()">
        <input id="fId" name="fId" type="hidden" value="${food.fId }"/>
        <!-- 第二层，添加公司信息的表格信息 -->
        <div align="center" style="height:350px;">
           <table border="1px" style="border-collapse: collapse;height:100%;">
             <tr style="height:80px;">
               ...
               <td><input id="fName" name="fName" maxlength="20" title="长度不超过20" value="${food.fName }"/></td>
               <td><font color="red">*</font>食品生产日期：</td>
               <td><input id="fBirthtime" name="fBirthtime" maxlength="10" title="请输入标准的日期格式（2018-02-20）" value="${food.fBirthtime }"/></td>
               <td><font color="red">*</font>食品保质日期：</td>
               <td><input id="fEndtime" name="fEndtime" maxlength="10" title="请输入标准的日期格式（2018-02-20）" value="${food.fEndtime }"/></td>
             </tr>
             <tr style="height:80px;">
               ...
               <td>是否准入：</td>
               <td><input id="fZhunRu" name="fZhunRu" value="${food.fZhunRu }" readonly="readonly"/> </td>
             </tr>
             <tr style="height:100px;">
               <td><font color="red">*</font>食品生产许可证：</td>
               <td><input type="text" name="fShengChanPiCiId" id="fShengChanPiCiId" maxlength="10" title="输入只能是数字" value="${food.fShengChanPiCiId }" readonly="readonly"/></td>
               ...
               <td rowspan="2">
                   <img src="../houtai/images/${food.fImage }" id="img" width="100px" height="120px"><br/>
	              <!-- file的name值不能和实体类一样对象名一致
	               <input type="button" name="genggai" id="genggai" value="更改照片" onclick="genggai()"/>
	               <input type="file" name="file" onchange="yuLan(this)" disabled="disabled" style="display:none"/> -->
	           </td>
             </tr>
             <tr style="height:80px;">
               ...
               <td>录入信息员工工号： </td>
               <td><input type="text" name="fInfoEmployeeId" value="${food.fInfoEmployeeId }" readonly="readonly"/> </td>
             </tr>
           </table>
        </div>
        <!-- 第三层，提交表单信息按钮-->
